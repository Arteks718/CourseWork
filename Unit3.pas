...
begin
  ...
  ShowMessage('Неможливо змінити користувача. Спочатку закрийте чек.')
...
+' ON Товар.Код_товара = Продажа.Код_товара) ON Чек.Код_чеку = Продажа.Код_чеку '
+' WHERE (((Продажа.Код_чеку)='+Cheknom+')) ';
  DataModule1.ADOQCheckRep.Open;
  ...
  Form7.QRLabel12.Caption:= 'Сумма по чеку ' + Label9.Caption + ' грн';
...
begin
  if Label1.Caption = '' then ShowMessage('Виберіть товар')
  else
  begin
    DataModule1.ADOQWork.Close;
    ...
    Label9.Caption := SumChek;
    Form8.Label1.Caption := 'Сумма по чеку (грн): ' + SumChek;
